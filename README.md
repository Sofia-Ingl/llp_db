...
```
NORMALIZATION_RESTRICT - запрет нормализации файла при исполнении конкретной команды. Поскольку нормализация представляет собой перезапись файла, она замедляет исполнение конкретной команды, хотя в дальнейшем позитивно сказывается на производительности (обеспечивая 100% попадание в буфер). Но если надо, чтобы update/delete исполнилась как можно быстрее, стоит нормализацию запретить. NORMALIZATION_ALLOW - после команды файл будет перезаписан, если пороговые значения фрагментации достигнуты. NORMALIZATION_DO_FORCE - нормализация будет выполнена вне зависимости от показателей фрагментации.
